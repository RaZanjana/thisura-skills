...
Resolve files via `adapters/bmad-inputs.md` first (BMAD/WDS auto-find). Treat legacy names the same:

- **Journeys / scenarios → the spine.** (Was: User Journey file.) This drives everything. Extract
  the **ordered list of journeys** exactly as they appear (preserve file order — never reorder).
  For each journey, pull its **steps** (the sequence of user actions/decisions) and any **branches**
  (alternate paths, conditions). The journey list is *not* asked of the user — only exclusions are.
- **PRD → requirements & rules.** What each screen must let the user do; field rules, validation,
  ...
  from.
- **Epics & stories → feature scope (coverage).** (Was: Themes & Epics / T&E.) The full feature
  inventory. Used to check that every in-scope feature a journey touches actually appears as a
  screen/element. **If the only source is Excel, use the worksheet the user named — never guess
  across sheets.** If they didn't name one and the file has several, stop and ask.
- **UX detail (optional) → layout & interaction.** EXPERIENCE.md / DESIGN.md, Freya page specs, or
  a similar UX notes file. If present, prefer it for layout decisions; if absent, derive layout
  from PRD + journeys and note the assumption.
...
documents** (PRD wording, epic/story grouping, journey context, UX detail) and **present that map
...
- **Source trace** — the epic/feature (epics & stories), requirement (PRD), and journey step each screen **and
...
- Every **journey** in the journeys/scenarios docs is either built or explicitly excluded.
...
annotations are drawn, check it back against PRD + epics & stories + journeys (+ UX detail) and
**fix issues before involving the user**:
- [ ] **Steps covered** — every step in this journey (from the journeys/scenarios docs) has a screen.
...
- [ ] **Features represented** — every epic/story feature this journey touches appears on a screen.
- [ ] **Copy traces** — meaningful labels reflect the PRD/journey wording; nothing invented.
- [ ] **No em dashes** — no `—` or `–` in any generated copy (box titles, bullets, stickies,
      connector labels, Mode B UI text, chat to the designer); use commas, periods, or
      parentheses.
